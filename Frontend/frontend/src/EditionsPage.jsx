...
function LoadingScreen() {
  return (
    <motion.div
      className="ed-loading"
      initial={{ opacity: 1 }}
      exit={{ opacity: 0, transition: { duration: 0.85 } }}
    >
      <ParticleCanvas />
      <motion.div
        className="ed-loading-logo"
...
function ApiDemoPanel({ edition }) {
  ...
  const callApi = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/tasks?size=3&page=0`, {
        headers,
      });
  ...
  return (
    <div className="api-demo" style={{ "--demo-color": edition.color1 }}>
      ...
      <div className="api-demo-tabs">
        <button
          className={`api-tab${tab === "preview" ? " active" : ""}`}
          onClick={() => setTab("preview")}
        >
          Preview
        </button>
        <button
          className={`api-tab${tab === "raw" ? " active" : ""}`}
          onClick={() => setTab("raw")}
        >
          JSON Raw
        </button>
      ...
      <div className="api-demo-body">
        {status === "idle" && (
          <p className="api-demo-hint">
            Nhấn <strong>Thử ngay</strong> để gọi API backend thật.
          </p>
        )}
        {status === "loading" && (
          ...
          <div className="api-preview-list">
            {data.data.content.map((task, i) => (
              <div key={i} className="api-task-row">
                <span
                  className={`task-status-dot status-${task.status?.toLowerCase()}`}
                />
...
function EditionSection({ edition, idx, onActive, isLast }) {
  ...
  useEffect(() => {
    if (isInView) onActive(idx);
  }, [isInView, idx, onActive]);
  ...
  return (
    <section ref={ref} className="ed-section" id={`edition-${edition.id}`}>
      <div className={`ed-section-inner${idx % 2 === 1 ? " reverse" : ""}`}>
        ...
        >
          <div
            className="ed-visual-card"
            style={{ "--c1": edition.color1, "--c2": edition.color2 }}
          >
            <motion.div
              className="ed-visual-glow"
              animate={
                isInView
                  ? { scale: [0.9, 1.2, 0.95], opacity: [0.3, 0.52, 0.3] }
                  : {}
              }
            ...
            <div className="ed-visual-grid-bg" aria-hidden="true">
              {Array.from({ length: 12 }).map((_, i) => (
                <motion.div
                  key={i}
                  className="ed-grid-cell"
                  animate={
                    isInView
                      ? {
                          opacity: [0, 0.18 + (i % 3) * 0.08, 0],
                          scale: [0.7, 1.05, 0.7],
                        }
                      : {}
                  }
            ...
            <span className="ed-visual-season-label">
              {edition.season} {edition.year}
            </span>

            <motion.span
              className="ed-visual-icon"
              aria-hidden="true"
              animate={
                isInView ? { scale: [1, 1.08, 1], rotate: [0, 4, -4, 0] } : {}
              }
            ...
            {/* Endpoint chip */}
            <motion.div
              className="ed-endpoint-chip"
              initial={{ opacity: 0, y: 10 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.7, duration: 0.6 }}
            >
              <span className="ed-endpoint-method">GET</span>
              <span className="ed-endpoint-path">
                {edition.endpoint.replace(/^(POST|GET|PUT|DELETE)\s/, "")}
              </span>
            </motion.div>
          </div>
        </motion.div>
        ...
        >
          <motion.div variants={itemV} className="ed-eyebrow">
            <span
              className="ed-eyebrow-dot"
              style={{ background: edition.color1 }}
            />
            {edition.season} · {edition.year}
          </motion.div>

          <motion.h2 variants={itemV} className="ed-tagline">
            {edition.tagLine}
          </motion.h2>
          <motion.p
            variants={itemV}
            className="ed-headline"
            style={{ color: edition.color1 }}
          >
            {edition.headline}
          </motion.p>
          <motion.p variants={itemV} className="ed-desc">
            {edition.description}
          </motion.p>
          ...
          <motion.ul variants={itemV} className="ed-feat-list">
            {edition.features.map((f, fi) => (
              <motion.li
                key={f.label}
                className="ed-feat-item"
                variants={itemV}
                custom={fi}
              >
                <span
                  className="ed-feat-icon"
                  style={{ background: edition.color1 }}
                >
                  ✓
                </span>
          ...
          {API_ROUTES.map((r, i) => (
            ...
            >
              <span
                ...
                style={{ color: METHOD_COLORS[r.method] }}
              >
                {r.method}
              </span>
        ...
        {STATS.map((s) => (
          <motion.div key={s.label} variants={itemV} className="ed-stat">
            <div className="ed-stat-num">
              {s.num}
              <span className="ed-stat-suffix">{s.suffix}</span>
            </div>
...
function BottomStrip({ activeIdx, setActiveIdx }) {
  ...
  return (
    <motion.nav
      className="ed-strip"
      aria-label="Điều hướng phiên bản"
      ...
      <div className="ed-strip-inner">
        {EDITIONS.map((ed, i) => (
          ...
          >
            ...
            <span className="ed-strip-meta">
              {ed.year} · {ed.season}
            </span>
            <span className="ed-strip-name">{ed.tagLine}</span>
            <motion.span
              className="ed-strip-bar"
              animate={{
                scaleX: i === activeIdx ? 1 : 0,
                background: ed.color1,
              }}
        ...
        <div className="ed-nav-inner">
          <a href="/" className="ed-logo" aria-label="Manage Task home">
            <motion.span
              ...
              animate={{
                background: `linear-gradient(135deg,${active.color1},${active.color2})`,
              }}
            ...
            <span className="ed-logo-sep" aria-hidden="true">
              |
            </span>
        ...
        <section className="ed-hero" aria-label="Hero">
          ...
          >
            ...
            <motion.p variants={itemV} className="ed-hero-sub">
              Từ JWT authentication đến profile người dùng — mỗi phiên bản là
              một bước tiến thực sự của{" "}
              <strong style={{ color: "#fff" }}>Manage Task</strong>. Spring
            ...
            <motion.div variants={itemV} className="ed-hero-stack">
              {[
                ...
                <span key={t} className="ed-stack-chip">
                  {t}
                </span>
              ))}
            </motion.div>
